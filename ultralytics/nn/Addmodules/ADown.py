...
class DCDConv(nn.Module):

    def __init__(self, c1, c2, stride=2):  # ch_in, ch_out, shortcut, kernels, groups, expand
        super().__init__()
        self.stride = stride
        self.c = c2 // 2
        self.Focus = Focus(c1 // 2, self.c, ksize=1, stride=1)
        self.cv2 = Conv(c1 // 2, self.c, 1, 1, 0)
        self.module = nn.SiLU(inplace=False)
    def forward(self, x):
